# Essentia CLI tools - works everywhere including headless
{
  pkgs,
  lib,
  config,
...
let
  cfg = config.profiles.cli.core;
in
{
  options.profiles.cli.core = {
    enable = lib.mkEnableOption "essential CLI tools (ripgrep, fd, bat, jq, etc.)";
    hasNvidia = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether this system has an NVIDIA GPU (uses btop-cuda instead of btop).";
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    home.packages = with pkgs; [
      ...
      (if cfg.hasNvidia then btop-cuda else btop) # system monitor
